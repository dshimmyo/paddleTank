...
#print ($amp0. " : buggy amp0\n");
my $amp1 = ($amplitude - 1) * 257;
if ($amp1 < 0) {$amp1 = 0;};

my $hexAmp0 = sprintf("0x%X", $amp0);
my $hexAmp1 = sprintf("0x%X", $amp1);
my $hexAmp0Special = sprintf("0x%X", $amp0Special);
#print ($hexAmp0. " : buggy hexAmp0\n");
my $packedHexAmp0 = pack('s<', hex($hexAmp0));
#print unpack ('H*',$packedHexAmp0 );
#print " : buggy packed hexAmp0\n";
...
for (my $i=0; $i<$framelength;$i++){
    ...
    if ($i==0) {
        print $out pack('s<', hex($hexAmp0Special) );
    }
    else 
    {
